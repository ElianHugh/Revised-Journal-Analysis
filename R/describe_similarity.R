...
describe_similarity <- function(boot.out) {
    ...
    boot_SE <- sd(boot.out$Mean)
    boot_SD <- mean(boot.out$SD)
    boot_M <- mean(boot.out$Mean)
    boot_M_CI <- bca(boot.out$Mean, conf.level = 0.99)
    boot_SD_CI <- bca(boot.out$SD, conf.level = 0.99)
